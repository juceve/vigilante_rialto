...
<div wire:poll.60s="verificarPendientes">
    @if ($tienePendientes && $intervaloActual && session('asistencia_operador'))
        <a href="{{ route('vigilancia.hombre-vivo', $intervaloActual['id']) }}" class="btn btn-link position-relative p-2"
            title="Marcar Hombre Vivo">
            <i class="fas fa-heartbeat fa-lg text-{{ $intervaloActual['urgente'] ? 'danger' : 'warning' }} pulse"></i>
            <span
                class="position-absolute top-0 start-100 translate-middle badge rounded-pill bg-{{ $intervaloActual['urgente'] ? 'danger' : 'warning' }}">
                1
